...
from numba import njit, boolean, uint8, int32, float32
...
@njit(uint8[:,:,:](uint8[:,:,:], uint8[:,:,:], float32[:,:,:], float32), cache=True)
def helper(source_frame, target_frame, MV_field, dist):
    ...
    for u in range(0, source_frame.shape[0]):
        for v in range(0, source_frame.shape[1]):
            ...
            u_i = int(u + round(MV_field[u,v,0]*dist))
            v_i = int(v + round(MV_field[u,v,1]*dist))
            # print("u_i ",u_i," v_i ",v_i)
            if(u_i<source_frame.shape[0] and v_i<source_frame.shape[1]):
                if MV_field[u,v,2] <= SAD_interpolated_frame[u_i, v_i]:

                    Interpolated_Frame[u_i, v_i] =  source_frame[u, v]
                    SAD_interpolated_frame[u_i, v_i] = MV_field[u,v,2]
            else:
                Interpolated_Frame[u, v] = source_frame[u, v]
                SAD_interpolated_frame[u, v] = MV_field[u, v, 2]
...
class UniDirInterpolator(MEMCIBaseInterpolator):
    ...
    def get_interpolated_frame(self, idx):
        ...
        return helper(source_frame, target_frame, self.MV_field, dist)
