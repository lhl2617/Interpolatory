...
@jit(float32[:](int8, float32[:,:,:], uint8[:,:,:], types.UniTuple(uint8, 2), int32, types.UniTuple(uint32, 3)), nopython=True)
...
@jit(float32[:,:,:](int8, int32, int32, types.UniTuple(uint8, 3), uint8[:,:,:], uint8[:,:,:]), nopython=True)
def full_search_helper(cost_key, block_size, win_size, im_shape, im1_pad, im2_pad):
    ...
    for row in range(mvs.shape[0]):
        ...
        for col in range(mvs.shape[1]):
            im_c = col * block_size
            block1 = np.asarray(im1_pad[im_r: im_r + block_size,
                             im_c: im_c + block_size, :], dtype=np.float32)
...
def full_search(cost_key, block_size, win_size, im1, im2):
    ...
    return full_search_helper(cost_key, block_size, win_size, im_shape, im1_pad, im2_pad)


@jit(float32[:,:,:](int8, int32, int32, uint8[:,:,:], uint8[:,:,:]), nopython=True)
def increase_vec_density(cost_key, mvs, block_size, sub_win_size, im1, im2, vec_scale=1):
    im1_pad = np.pad(im1, ((0, block_size), (0, block_size), (0, 0)))
    im2_pad = np.pad(im2, ((0, block_size), (0, block_size), (0, 0)))

    out = np.zeros((mvs.shape[0] << 1, mvs.shape[1]
                    << 1, mvs.shape[2]), dtype='float32')
        ...
        for col in range(mvs.shape[1]):
            ...
            for o_r in range(row*2, r_max):
                ...
                for o_c in range(col*2, c_max):
                    ...
                    for vec in vecs:
                        res = block_wise_fs(
                            cost_key, block, im2_pad, (im_r+vec[0], im_c+vec[1]), sub_win_size, im1.shape)
