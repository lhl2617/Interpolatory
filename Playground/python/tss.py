...
def get_motion_vectors_tss(block_size, steps, source_frame, target_frame):
    ...
    for s_row in range(0, source_frame.shape[0], block_size):
        ...
        for s_col in range(0, source_frame.shape[1], block_size):
            ...
            curr_block_size = source_block[:,:,0].shape
            nr_of_pxl=curr_block_size[0]*curr_block_size[1]
            center_sad = None
                ...
                for t_row in range(curr_row - S, curr_row + S + 1, S):
                    for t_col in range(curr_col - S,curr_col + S + 1, S):
                        ...
                        if (t_row != curr_row or t_col != curr_col or center_sad == None) and (t_row >= 0 and t_row <= source_frame.shape[0] - block_size and t_col >= 0 and t_col <= source_frame.shape[1] - block_size):
                            target_block = target_frame[t_row:t_row+curr_block_size[0], t_col:t_col+curr_block_size[1], :]
            ...
            block = np.full((curr_block_size[0], curr_block_size[1], 3), [lowest_idx[0] - s_row, lowest_idx[1] - s_col, lowest_sad])
            output[s_row:s_row+curr_block_size[0], s_col:s_col+curr_block_size[1], :] = block
